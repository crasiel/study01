	      ...
	      <ul class="nav navbar-nav navbar-right">
            ...
            <li><a href="${path }/memLogout.do"><span class="glyphicon glyphicon-log-out"></span> 로그아웃</a></li>
		  	<c:if test="${empty mvo.memProfile}">
		  		<li><img src="${path }/resources/images/person.png" class="img-circle" style="width : 50px; height : 50px;"> ${mvo.memName }님
		  		(
		  	...
		  	<c:if test="${!empty mvo.memProfile}">
		  		<li><img src="${path }/resources/upload/${mvo.memProfile}" class="img-circle" style="width : 50px; height : 50px"> ${mvo.memName }님 
		  		(
		  		<c:forEach var="authVO" items="${mvo.authList }">
		  			<c:if test="${authVO.auth eq 'ROLE_USER' }">U</c:if>
		  			<c:if test="${authVO.auth eq 'ROLE_MANAGER' }">M</c:if>
